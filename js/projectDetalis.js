
//FUNCTIONS AND OBJECTS

function getCurrentProjeto(id) {
    let response;
    let x = new XMLHttpRequest();
    x.onload = () => {
        console.log(x.responseText);
        console.log(JSON.parse(x.responseText));
        loadData(JSON.parse(x.responseText));
    }

    x.onerror = (err) => {
        console.log(err);
    }

    x.open('GET', './server/actions/getCurrentProjeto.php?id=' + id);
    x.send();
}

function loadData(response) {

        document.getElementById('author').innerText = response[0].name;
        document.getElementById('title').innerText = response[0].title;
        document.getElementById('desc').innerText = response[0].description;
        document.getElementById('date').innerText = response[0].created_at;
        document.getElementById('banner').src= response[0].banner;

}

function getComments(projectId) {

    let response;
    let x = new XMLHttpRequest();

    x.onload = () => {
        console.log(x.responseText);
        console.log(JSON.parse(x.responseText));
        loadComments(JSON.parse(x.responseText));
    }

    x.onerror = (err) => {
        console.log(err);
    }
    console.log(`http://localhost/EmpreendedorismoServer/server/actions/getComments.php?id=${projectId}`);
    x.open('GET', `./server/actions/getComments.php?id=${projectId}`);
    x.send();
}


function loadComments(response) {
    let commentContainer = document.getElementById('commentContainer');

    for (let i = 0; i < response.length; i++) {
        commentContainer.innerHTML += `<ul class="user_comment">
        <div class="row">
            <div class="col-12">
                <div class="user_avatar">
                    <img
                        src="data:image/jpeg;base64,">

                </div><!-- the comment body -->
                <p style="display: inline; vertical-align: middle;">${response[i].name}</p>
            </div>
        </div>
        <div class="row">
            <div class="comment_body">
                <p style="padding:10px;">${response[i].comment}</p>
                <div style="padding-left: 10px;" class="row">
                </div>
            </div>
        </div>




        <!-- comments toolbar -->
        <div class="comment_toolbar">

            <!-- inc. date and time -->
            <div class="comment_details">
                <ul>
                    <li><i class="fa fa-clock-o"></i> 13:94</li>
                    <li><i class="fa fa-calendar"></i> 04/01/2015</li>

                    </li>
                </ul>
            </div><!-- inc. share/reply and love -->
            <div class="comment_tools">
                <ul>
                    <li><i class="fa fa-share-alt"></i></li>
                    <li><i class="fa fa-reply"></i></li>
                    <li><i class="fa fa-heart love"></i></li>
                </ul>
            </div>

        </div>
    </ul>`;
    }


}

//MAIN
{
    var currentProject=sessionStorage.getItem("currentProject");

    if (sessionStorage.getItem("currentProject") != null) {
        getCurrentProjeto(sessionStorage.getItem("currentProject"));
        //getComments(sessionStorage.getItem("currentProject"));
        //sessionStorage.removeItem("currentProject");
    }
    else {
        window.location.href = "./index.html";
    }

    window.onload=()=>{
        getComments(sessionStorage.getItem("currentProject"));
    }


}